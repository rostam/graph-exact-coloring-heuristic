...
\title{\textbf{Two Graph Coloring Algorithms Based on Independent Sets:\\
IndSet-Based Exact Coloring and IndSet-Removal Coloring}}
...
\begin{abstract}
We describe and analyze two graph coloring algorithms both rooted in the
independent-set structure of a graph.  The first, \emph{IndSet-Based exact
coloring}, determines the chromatic number exactly by evaluating a
combinatorial quantity derived from inclusion-exclusion over all independent
sets.  The quantity, denoted $c_k(G)$, satisfies $c_k(G) = 0$ if and only if
$k < \chi(G)$, so the chromatic number equals the smallest $k$ for which
$c_k(G) \neq 0$.  The second, \emph{IndSet-Removal Coloring}, repeatedly peels
off one independent set per round and assigns it the next available color,
yielding a fast upper bound on $\chi(G)$.  Both algorithms are implemented in
C++17 using the Boost Graph Library.  We present experimental results on
cycles $C_n$, complete graphs $K_n$, the Petersen graph $GP(5,2)$ and other
generalized Petersen graphs, wheel graphs $W_n$, complete multipartite graphs
$K_{s,\ldots,s}$, edgeless graphs, and graphs from the SuiteSparse Matrix
Collection, reporting both the exact chromatic number and the IndSet-Removal
upper bound side by side.  The exact algorithm is practical for graphs up to
roughly 13--14 vertices on a single core; IndSet-Removal runs in $O(n \cdot m)$
and is applicable to larger instances.
\end{abstract}
...
This paper documents two algorithms implemented in the
\texttt{graph-exact-coloring-heuristic} project.
The first, \emph{IndSet-Based Exact Coloring}, uses the core function
\texttt{c\_k} which evaluates the inclusion-exclusion sum
\[
...
The second, \emph{IndSet-Removal Coloring} (implemented in
...
correctness.  Section~\ref{sec:heuristic} describes the IndSet-Removal Coloring algorithm.
...
\begin{definition}[Independent set cover]
  An \emph{ordered $k$-cover by independent sets} (hereafter, $k$-IndSet-cover) is
  a $k$-tuple $(I_1, \ldots, I_k) \in \mathcal{F}(G)^k$ such that
  $I_1 \cup \cdots \cup I_k = V$.
\end{definition}

Note that in a $k$-IndSet-cover the sets need not be disjoint, and they may be
empty.  A proper $k$-coloring induces a $k$-IndSet-cover (the color classes), but
a $k$-IndSet-cover may have overlapping sets.  We will show these notions are
...
\begin{theorem}\label{thm:ck-cover}
  $c_k(G,k)$ equals the number of ordered $k$-IndSet-covers of $G$.
\end{theorem}
...
$k$-IndSet-cover.  Hence $c_k(G,k) \geq 1 > 0$.

$(\Rightarrow)$ Suppose $k < \chi(G)$.  We show no $k$-IndSet-cover exists.
If $(I_1,\ldots,I_k)$ were a $k$-IndSet-cover, define $f(v) = \min\{j : v \in I_j\}$
...
\begin{remark}
  ...
  $k$-IndSet-cover induces a proper coloring immediately), but they differ for
...
% ─────────────────────────────────────────────────────────────────────────────
\section{IndSet-Removal Coloring}\label{sec:heuristic}
% ─────────────────────────────────────────────────────────────────────────────

\subsection{Independent Set Extraction}

The IndSet extraction procedure (Algorithm~\ref{alg:greedyIS}) builds one
...
IndSet-Removal Coloring produces at most $\Delta(G)+1$ colors, where $\Delta(G)$
...
on bipartite or dense multipartite graphs (see Table~\ref{tab:results} for
...
\begin{sloppypar}
The project uses CMake 3.10 or later.  Three targets are built:
\texttt{indset\_exact\_coloring} (IndSet-Based Exact Coloring demo,
\texttt{demo\_indset\_exact\_coloring.cpp}),
\texttt{indset\_removal\_coloring} (IndSet-Removal demo,
\texttt{demo\_indset\_removal\_coloring.cpp}), and \texttt{tests}
(\texttt{tests.cpp}, the test suite described in
...
summarizes results for both the IndSet-Based Exact Coloring and IndSet-Removal Coloring algorithms.

\begin{table}[ht]
\centering
\caption{All three results per graph: known $\chi$, IndSet-Based Exact Coloring,
  and IndSet-Removal Coloring (78 exact assertions, all passing).}
\label{tab:results}
\begin{tabular}{llccccc}
\toprule
Graph & Description & $|V|$ & $|E|$ & \makecell{Known\\$\chi$} & \makecell{IndSet-\\Exact} & \makecell{IndSet-\\Removal} \\
\midrule
\multicolumn{7}{l}{\textit{Cycle graphs}} \\
$C_3$ & Triangle                    & 3  & 3   & 3  & 3  & 3  \\
$C_4$ & 4-cycle (square)            & 4  & 4   & 2  & 2  & 4  \\
$C_5$ & 5-cycle (pentagon)          & 5  & 5   & 3  & 3  & 3  \\
$C_6$ & 6-cycle (hexagon)           & 6  & 6   & 2  & 2  & 4  \\
$C_7$ & 7-cycle                     & 7  & 7   & 3  & 3  & 3  \\
\midrule
\multicolumn{7}{l}{\textit{Complete and edgeless graphs}} \\
$K_1$ & Single vertex               & 1  & 0   & 1  & 1  & 1  \\
$K_2$ & Single edge                 & 2  & 1   & 2  & 2  & 2  \\
$K_3$ & Triangle                    & 3  & 3   & 3  & 3  & 3  \\
$K_4$ & Complete graph              & 4  & 6   & 4  & 4  & 4  \\
$K_9$ & Complete graph              & 9  & 36  & 9  & 9  & 9  \\
$K_{10}$ & Complete graph           & 10 & 45  & 10 & 10 & 10 \\
$E_1$ & Isolated vertex             & 1  & 0   & 1  & 1  & 1  \\
$E_3$ & Three isolated vertices     & 3  & 0   & 1  & 1  & 1  \\
\midrule
\multicolumn{7}{l}{\textit{Bipartite graphs}} \\
$K_{2,3}$ & Complete bipartite      & 5  & 6   & 2  & 2  & 3  \\
\midrule
\multicolumn{7}{l}{\textit{Wheel graphs} $W_n$; $\chi=3$ if $n$ odd, $\chi=4$ if $n$ even} \\
$W_5$ & Hub $+\,C_4$ (even rim)     & 5  & 8   & 3  & 3  & 5  \\
$W_6$ & Hub $+\,C_5$ (odd rim)      & 6  & 10  & 4  & 4  & 4  \\
$W_7$ & Hub $+\,C_6$ (even rim)     & 7  & 12  & 3  & 3  & 5  \\
$W_8$ & Hub $+\,C_7$ (odd rim)      & 8  & 14  & 4  & 4  & 4  \\
$W_9$ & Hub $+\,C_8$ (even rim)     & 9  & 16  & 3  & 3  & 5  \\
\midrule
\multicolumn{7}{l}{\textit{Complete multipartite graphs};
  $\chi(K_{s,\ldots,s}^{(k)}) = k$} \\
$K_{2,2,2}$    & Octahedron          & 6  & 12  & 3  & 3  & 5  \\
$K_{3,3,3}$    & Complete 3-partite  & 9  & 27  & 3  & 3  & 7  \\
$K_{2,2,2,2}$  & Complete 4-partite  & 8  & 24  & 4  & 4  & 7  \\
$K_{2\times5}$ & Complete 5-partite  & 10 & 40  & 5  & 5  & 9  \\
$K_{2\times6}$ & Complete 6-partite  & 12 & 60  & 6  & 6  & 11 \\
$K_{2\times7}$ & Complete 7-partite  & 14 & 84  & 7  & 7  & 13 \\
$K_{2\times8}$ & Complete 8-partite  & 16 & 112 & 8  & 8  & 15 \\
$K_{2\times9}$ & Complete 9-partite  & 18 & 144 & 9  & 9  & 17 \\
\midrule
\multicolumn{7}{l}{\textit{Generalized Petersen graphs} $GP(n,k)$} \\
$GP(4,1)$ & 3-cube $Q_3$ (bipartite) & 8  & 12  & 2  & 2  & 6  \\
$GP(5,1)$ & Pentagonal prism         & 10 & 15  & 3  & 3  & 3  \\
$GP(5,2)$ & Petersen graph           & 10 & 15  & 3  & 3  & 4  \\
$GP(6,1)$ & Hexagonal prism          & 12 & 18  & 2  & 2  & 6  \\
$GP(6,2)$ & ---                      & 12 & 18  & 3  & 3  & 4  \\
$GP(7,2)$ & ---                      & 14 & 21  & 3  & 3  & 4  \\
\bottomrule
\end{tabular}
...
All 78 test assertions pass: IndSet-Based Exact Coloring recovers the known
chromatic number in every case.  In particular, $c_k(K_9, 9) = 9! = 362{,}880$
and $c_k(K_{10}, 10) = 10! = 3{,}628{,}800$ are verified exactly.
IndSet-Removal Coloring matches the exact result on complete graphs and odd
cycles, but over-estimates on even cycles, bipartite, and multipartite graphs.
...
\begin{table}[h]
\centering
\caption{SuiteSparse graphs: known $\chi$, IndSet-Based Exact Coloring,
  IndSet-Removal Coloring, and wall-clock time for each algorithm.}
\label{tab:suitesparse}
{\footnotesize
\begin{tabular}{lllccc ccc c}
\toprule
Matrix & Group & Processing & $|V|$ & $|E|$ & \makecell{Known\\$\chi$} & \makecell{IndSet-\\Exact} & \makecell{IndSet-\\Removal} & \makecell{Exact\\Time} & \makecell{Removal\\Time} \\
\midrule
\texttt{cage3}   & vanHeukelum & symmetrized (full) & 5  &  7 & 3 & 3 & 4 & $<$1\,ms & $<$1\,ms \\
\texttt{cage4}   & vanHeukelum & symmetrized (full) & 9  & 20 & 3 & 3 & 7 &    3\,ms & $<$1\,ms \\
\texttt{GD98\_a} & Pajek       & BFS-10 subgraph    & 10 & 10 & 3 & 3 & 3 &   81\,ms & $<$1\,ms \\
\texttt{ndtest}  & (project)   & BFS-10 subgraph    & 10 & 14 & 2 & 2 & 6 &   15\,ms & $<$1\,ms \\
\bottomrule
\end{tabular}}
\end{table}
...
\caption{Wall-clock time for larger graphs (Release build, single core, Ubuntu 22.04).
  IndSet-Exact gives the exact chromatic number; IndSet-Removal is the heuristic upper bound.}
\label{tab:scaling}
\begin{tabular}{llccccccc}
\toprule
Graph & Family & $|V|$ & $|E|$ & \makecell{Known\\$\chi$} & \makecell{IndSet-\\Exact} & \makecell{IndSet-\\Removal} & \makecell{Exact\\Time} & \makecell{Removal\\Time} \\
\midrule
$C_8$     & Cycle          &  8 &  8 & 2 & 2 & 4 &    1\,ms & $<$1\,ms \\
$C_9$     & Cycle          &  9 &  9 & 3 & 3 & 3 &    7\,ms & $<$1\,ms \\
$C_{10}$  & Cycle          & 10 & 10 & 2 & 2 & 4 &   24\,ms & $<$1\,ms \\
$C_{11}$  & Cycle          & 11 & 11 & 3 & 3 & 3 &   86\,ms & $<$1\,ms \\
$C_{12}$  & Cycle          & 12 & 12 & 2 & 2 & 4 &  168\,ms & $<$1\,ms \\
$C_{13}$  & Cycle          & 13 & 13 & 3 & 3 & 3 &  786\,ms & $<$1\,ms \\
$C_{14}$  & Cycle          & 14 & 14 & 2 & 2 & 4 & $\approx$3\,s & $<$1\,ms \\
$C_{16}$  & Cycle          & 16 & 16 & 2 & 2 & 4 & $\approx$33\,s & $<$1\,ms \\
\midrule
$K_5$     & Complete       &  5 & 10 & 5 & 5 & 5 & $<$1\,ms & $<$1\,ms \\
$K_6$     & Complete       &  6 & 15 & 6 & 6 & 6 & $<$1\,ms & $<$1\,ms \\
$K_7$     & Complete       &  7 & 21 & 7 & 7 & 7 & $<$1\,ms & $<$1\,ms \\
$K_8$     & Complete       &  8 & 28 & 8 & 8 & 8 &    1\,ms & $<$1\,ms \\
\midrule
$K_{4,4}$ & Bipartite      &  8 & 16 & 2 & 2 & 5 &    1\,ms & $<$1\,ms \\
$K_{5,5}$ & Bipartite      & 10 & 25 & 2 & 2 & 6 &    9\,ms & $<$1\,ms \\
$K_{6,6}$ & Bipartite      & 12 & 36 & 2 & 2 & 7 &   69\,ms & $<$1\,ms \\
$K_{7,7}$ & Bipartite      & 14 & 49 & 2 & 2 & 8 &  536\,ms & $<$1\,ms \\
\midrule
$GP(6,2)$ & Gen.\ Petersen & 12 & 18 & 3 & 3 & 4 &  135\,ms & $<$1\,ms \\
$GP(7,2)$ & Gen.\ Petersen & 14 & 21 & 3 & 3 & 4 & 1333\,ms & $<$1\,ms \\
$GP(8,3)$ & Gen.\ Petersen & 16 & 24 & 2 & 2 & 5 & $\approx$15\,s & $<$1\,ms \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{High chromatic number graphs: known $\chi$, IndSet-Based Exact Coloring,
  IndSet-Removal Coloring, and wall-clock time for each algorithm.}
\label{tab:highchi}
\begin{tabular}{lccccccc}
\toprule
Graph & $V$ & $E$ & \makecell{Known\\$\chi$} & \makecell{IndSet-\\Exact} & \makecell{IndSet-\\Removal} & \makecell{Exact\\Time} & \makecell{Removal\\Time} \\
\midrule
$K_9$          & 9  & 36  & 9  & 9  & 9  &    3\,ms & $<$1\,ms \\
$K_{10}$       & 10 & 45  & 10 & 10 & 10 &    9\,ms & $<$1\,ms \\
$K_{2\times5}$ & 10 & 40  & 5  & 5  & 9  &    7\,ms & $<$1\,ms \\
$K_{2\times6}$ & 12 & 60  & 6  & 6  & 11 &   47\,ms & $<$1\,ms \\
$K_{2\times7}$ & 14 & 84  & 7  & 7  & 13 &  287\,ms & $<$1\,ms \\
$K_{2\times8}$ & 16 & 112 & 8  & 8  & 15 &  1.1\,s  & $<$1\,ms \\
$K_{2\times9}$ & 18 & 144 & 9  & 9  & 17 &  5.5\,s  & $<$1\,ms \\
\bottomrule
\end{tabular}
\end{table}

\subsection{IndSet-Removal vs.\ Exact Coloring}\label{sec:greedy_vs_exact}

The IndSet-Removal column in Tables~\ref{tab:results}--\ref{tab:highchi} shows
the number of colors produced by Algorithm~\ref{alg:greedycol} on each graph.
The algorithm processes vertices in ascending index order, so its quality
depends strongly on the vertex ordering.  The results illustrate several
patterns:

\begin{itemize}
\item \textbf{Complete graphs}: IndSet-Removal is always optimal.  Because every
  vertex is adjacent to all lower-indexed vertices, each vertex receives a new
  color, achieving exactly $n$ colors for $K_n$.

\item \textbf{Odd cycles}: IndSet-Removal is optimal ($\chi = 3$).  Vertex 0 and
  its non-adjacent neighbor both receive color 0, and the single remaining
  vertex requires a third color.

\item \textbf{Even cycles and bipartite graphs}: IndSet-Removal performs poorly
...
\item \textbf{Complete multipartite $K_{2\times k}$}: the over-count grows
  linearly with $k$; IndSet-Removal uses approximately $2k-1$ colors instead of
  $k$, giving an approximation ratio approaching 2 as $k$ grows.
\end{itemize}

These results confirm that IndSet-Removal Coloring is a fast but unreliable upper
...
IndSet-Based Exact Coloring of small subgraphs; for large graphs the IndSet-Removal
coloring in \texttt{demo\_is\_removal\_coloring.cpp} is the appropriate choice.
...
graphs or for IndSet-Based Exact Coloring of subgraphs within a larger pipeline.
The companion IndSet-Removal Coloring algorithm runs in $O(n + m)$ time per color
