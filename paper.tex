...
graphs $W_n$, complete multipartite graphs $K_{s,\ldots,s}$, and edgeless
graphs, confirming that the exact algorithm recovers the known chromatic
numbers for all tested families.  We also report empirical scaling data
showing that the algorithm is practical for graphs up to roughly 13--14
vertices on a single core.
\end{abstract}
...
\begin{table}[h]
\centering
\caption{Chromatic numbers computed by the exact algorithm (65 assertions, all passing).}
\label{tab:results}
...
All 65 test assertions pass, confirming both the zero/nonzero behavior of
...
\paragraph{Overflow limitation.}
The implementation accumulates the inclusion-exclusion sum in a 32-bit
\texttt{int}.  For dense complete graphs $K_n$ with $n \geq 9$, individual
terms of the form $(-1)^{|S|}\,\alpha(S,\mathcal{F})^k$ exceed $2^{31}$,
causing silent overflow.  Cycles and bipartite graphs are not affected because
their independent-set counts are much smaller.  Replacing \texttt{int} with
\texttt{int64\_t} would extend the safe range to $K_n$ with $n \lesssim 18$.
...
\texttt{main2.cpp} is the appropriate choice.
