...
IS-Based Exact Coloring and IS-Removal Coloring}}
...
independent-set structure of a graph.  The first, \emph{IS-based exact
...
$c_k(G) \neq 0$.  The second, \emph{IS-removal coloring}, repeatedly peels
...
Collection, reporting both the exact chromatic number and the IS-removal
upper bound side by side.  The exact algorithm is practical for graphs up to
roughly 13--14 vertices on a single core; IS-removal runs in $O(n \cdot m)$
...
The first, \emph{IS-based exact coloring}, uses the core function
...
The second, \emph{IS-removal coloring} (implemented in
...
\texttt{is\_exact\_coloring} (IS-based exact coloring demo,
\texttt{demo\_is\_exact\_coloring.cpp}),
\texttt{is\_removal\_coloring} (IS-removal demo,
\texttt{demo\_is\_removal\_coloring.cpp}), and \texttt{tests}
...
summarizes results for both the IS-based exact coloring and IS-removal coloring algorithms.

\begin{table}[ht]
\centering
\caption{Results for both algorithms (78 exact assertions, all passing).
  ``IS'' = IS-removal colors; $=$ means optimal, $>$ means over-estimate.}
\label{tab:results}
\begin{tabular}{llccccc}
\toprule
Graph & Description & $|V|$ & $|E|$ & $\chi$ & IS & IS vs.\ $\chi$ \\
\midrule
\multicolumn{7}{l}{\textit{Cycle graphs}} \\
$C_3$ & Triangle                    & 3  & 3   & 3  & 3  & $=$ \\
$C_4$ & 4-cycle (square)            & 4  & 4   & 2  & 4  & $+2$ \\
$C_5$ & 5-cycle (pentagon)          & 5  & 5   & 3  & 3  & $=$ \\
$C_6$ & 6-cycle (hexagon)           & 6  & 6   & 2  & 4  & $+2$ \\
$C_7$ & 7-cycle                     & 7  & 7   & 3  & 3  & $=$ \\
\midrule
...
$K_{2,3}$ & Complete bipartite      & 5  & 6   & 2  & 3  & $+1$ \\
\midrule
\multicolumn{7}{l}{\textit{Wheel graphs} $W_n$; $\chi=3$ if $n$ odd, $\chi=4$ if $n$ even} \\
$W_5$ & Hub $+\,C_4$ (even rim)     & 5  & 8   & 3  & 5  & $+2$ \\
$W_6$ & Hub $+\,C_5$ (odd rim)      & 6  & 10  & 4  & 4  & $=$ \\
$W_7$ & Hub $+\,C_6$ (even rim)     & 7  & 12  & 3  & 5  & $+2$ \\
$W_8$ & Hub $+\,C_7$ (odd rim)      & 8  & 14  & 4  & 4  & $=$ \\
$W_9$ & Hub $+\,C_8$ (even rim)     & 9  & 16  & 3  & 5  & $+2$ \\
\midrule
\multicolumn{7}{l}{\textit{Complete multipartite graphs};
  $\chi(K_{s,\ldots,s}^{(k)}) = k$} \\
$K_{2,2,2}$    & Octahedron          & 6  & 12  & 3  & 5  & $+2$ \\
$K_{3,3,3}$    & Complete 3-partite  & 9  & 27  & 3  & 7  & $+4$ \\
$K_{2,2,2,2}$  & Complete 4-partite  & 8  & 24  & 4  & 7  & $+3$ \\
$K_{2\times5}$ & Complete 5-partite  & 10 & 40  & 5  & 9  & $+4$ \\
$K_{2\times6}$ & Complete 6-partite  & 12 & 60  & 6  & 11 & $+5$ \\
$K_{2\times7}$ & Complete 7-partite  & 14 & 84  & 7  & 13 & $+6$ \\
$K_{2\times8}$ & Complete 8-partite  & 16 & 112 & 8  & 15 & $+7$ \\
$K_{2\times9}$ & Complete 9-partite  & 18 & 144 & 9  & 17 & $+8$ \\
\midrule
\multicolumn{7}{l}{\textit{Generalized Petersen graphs} $GP(n,k)$} \\
$GP(4,1)$ & 3-cube $Q_3$ (bipartite) & 8  & 12  & 2  & 6  & $+4$ \\
$GP(5,1)$ & Pentagonal prism         & 10 & 15  & 3  & 3  & $=$ \\
$GP(5,2)$ & Petersen graph           & 10 & 15  & 3  & 4  & $+1$ \\
$GP(6,1)$ & Hexagonal prism          & 12 & 18  & 2  & 6  & $+4$ \\
$GP(6,2)$ & ---                      & 12 & 18  & 3  & 4  & $+1$ \\
$GP(7,2)$ & ---                      & 14 & 21  & 3  & 4  & $+1$ \\
\bottomrule
\end{tabular}
...
The rightmost columns show the IS-removal coloring result alongside $\chi$ for
direct comparison.
...
\begin{table}[h]
\centering
\caption{Results on graphs derived from SuiteSparse matrices.
  IS = IS-removal colors; Time = IS-based exact coloring wall-clock time.}
\label{tab:suitesparse}
\begin{tabular}{lllcccccc}
\toprule
Matrix & Group & Processing & $|V|$ & $|E|$ & $\chi$ & IS & IS vs.\ $\chi$ & Time \\
\midrule
\texttt{cage3}   & vanHeukelum & symmetrized (full) & 5  &  7 & 3 & 4 & $+1$ & $<$1\,ms \\
\texttt{cage4}   & vanHeukelum & symmetrized (full) & 9  & 20 & 3 & 7 & $+4$ &    3\,ms \\
\texttt{GD98\_a} & Pajek       & BFS-10 subgraph    & 10 & 10 & 3 & 3 & $=$ &   81\,ms \\
\texttt{ndtest}  & (project)   & BFS-10 subgraph    & 10 & 14 & 2 & 6 & $+4$ &   15\,ms \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lccccc}
\toprule
Graph & $V$ & $E$ & $\chi$ & IS & Time \\
\midrule
$K_9$          & 9  & 36  & 9  & 9  & 3\,ms \\
$K_{10}$       & 10 & 45  & 10 & 10 & 9\,ms \\
$K_{2\times5}$ & 10 & 40  & 5  & 9  & 7\,ms \\
$K_{2\times6}$ & 12 & 60  & 6  & 11 & 47\,ms \\
$K_{2\times7}$ & 14 & 84  & 7  & 13 & 287\,ms \\
$K_{2\times8}$ & 16 & 112 & 8  & 15 & 1.1\,s \\
$K_{2\times9}$ & 18 & 144 & 9  & 17 & 5.5\,s \\
\bottomrule
\end{tabular}
\end{table}
...
The IS-removal column in Tables~\ref{tab:results}--\ref{tab:highchi} shows
...
IS-based exact coloring of small subgraphs; for large graphs the IS-removal
...
graphs or for IS-based exact coloring of subgraphs within a larger pipeline.
