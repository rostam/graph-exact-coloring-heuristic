...
describe a companion greedy heuristic that computes an upper bound on
...
The companion greedy algorithm, implemented in \texttt{main2.cpp}, repeatedly
extracts a greedy independent set and assigns it the next available color,
yielding an upper bound that is exact on bipartite and perfect graphs but may
...
correctness.  Section~\ref{sec:heuristic} describes the greedy heuristic.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{The Greedy Coloring Heuristic}\label{sec:heuristic}
% ─────────────────────────────────────────────────────────────────────────────

\subsection{Independent Set Extraction}

The greedy procedure (Algorithm~\ref{alg:greedyIS}) builds an independent set
by iterating through the active vertices in index order.  For each vertex $v$
selected, it removes $v$ and all of $v$'s current neighbors from the active
set.

\begin{algorithm}
\caption{Greedy independent set extraction}\label{alg:greedyIS}
\begin{algorithmic}[1]
...
color classes one at a time.

\begin{algorithm}
\caption{Greedy graph coloring}\label{alg:greedycol}
\begin{algorithmic}[1]
\Require Graph $G = (V, E)$
\Ensure Coloring array $\mathit{col}[0\ldots n-1]$
\State $\mathit{col} \gets$ array of size $n$; $k \gets 0$
\While{$G$ has edges}
  \State $I \gets \textsc{GreedyIndepSet}(G)$
  \For{each $v \in I$}
...
The greedy algorithm is well-known to produce at most $\Delta(G)+1$ colors,
where $\Delta(G)$ is the maximum degree~\cite{Welsh1967}.  It is exact on
bipartite graphs and on chordal graphs (with an appropriate vertex ordering),
but can be arbitrarily far from optimal on general graphs.  Its primary
advantage is $O(n + m)$ running time per color class, making it practical for
large sparse graphs.
...
\subsection{Greedy vs.\ Exact Coloring}\label{sec:greedy_vs_exact}

Table~\ref{tab:greedy} compares the number of colors produced by the greedy
heuristic (Algorithm~\ref{alg:greedycol}) against the true chromatic number
$\chi(G)$ on the same graph families.  The greedy processes vertices in
ascending index order, so its quality depends strongly on the vertex ordering.

\begin{table}[ht]
\centering
\caption{Greedy colors vs.\ exact chromatic number $\chi(G)$.
  ``='' means the greedy is optimal; ``$>$'' means it over-estimates.}
\label{tab:greedy}
\begin{tabular}{llcccc}
\toprule
Graph & Family & $\chi$ & Greedy & $\Delta(G)$ & Note \\
\midrule
...
\item \textbf{Complete multipartite $K_{2\times k}$}: the over-count grows
  linearly with $k$; the greedy uses approximately $2k-1$ colors instead of
  $k$, giving an approximation ratio approaching 2 as $k$ grows.
\end{itemize}

These results confirm that the greedy is a fast but unreliable upper bound: it
is tight on graphs where the natural vertex order aligns with the chromatic
structure (complete graphs, odd cycles), but can be far from optimal on
structured sparse or multipartite graphs.
...
exact coloring of small subgraphs; for large graphs the greedy heuristic in
\texttt{demo\_greedy\_coloring.cpp} is the appropriate choice.
...
The companion greedy coloring algorithm runs in linear time and provides a fast
upper bound useful as an initialization strategy or a standalone heuristic for
large instances.
